...
async function main() {
  // Connect to a local oryx server instance using insecure credentials
  const client = Client.connect("localhost:50051", insecureCredentials());

  const key = "example-greeting";
  const value = Buffer.from("Hello oryx from TypeScript!");
  ...
  try {
    ...
    console.error("An error occurred during ORYX operations:", error);
  } finally {
